...
#define Onfido_SEGMENT_API_BASE [NSURL URLWithString:@"https://api.segment.io/v1"]
#define Onfido_SEGMENT_CDN_BASE [NSURL URLWithString:@"https://cdn-settings.segment.com/v1"]
#define Onfido_MOBILE_SERVICE_BASE [NSURL URLWithString:@"https://mobile-service.segment.com/v1"]

NS_ASSUME_NONNULL_BEGIN


@interface OnfidoSEGHTTPClient : NSObject

@property (nonatomic, strong) OnfidoSEGRequestFactory requestFactory;
...
+ (OnfidoSEGRequestFactory)defaultRequestFactory;
+ (NSString *)authorizationHeader:(NSString *)writeKey;

- (instancetype)initWithRequestFactory:(OnfidoSEGRequestFactory _Nullable)requestFactory;
 ...
 */
- (NSURLSessionUploadTask *)upload:(ONFIDO_JSON_DICT)batch forWriteKey:(NSString *)writeKey completionHandler:(void (^)(BOOL retry))completionHandler;

- (NSURLSessionDataTask *)settingsForWriteKey:(NSString *)writeKey completionHandler:(void (^)(BOOL success, ONFIDO_JSON_DICT _Nullable settings))completionHandler;

- (NSURLSessionDataTask *)attributionWithWriteKey:(NSString *)writeKey forDevice:(ONFIDO_JSON_DICT)context completionHandler:(void (^)(BOOL success, ONFIDO_JSON_DICT _Nullable properties))completionHandler;
